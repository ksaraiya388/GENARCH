...
The first is collocation. DEQ's August 7, 2026 report publishes four sensor-versus-regulatory-monitor regressions at the collocated Ashburn site. Three of the four reproduce within 0.05 on the intercept and slope and 0.03 on R². The fourth, APEX 5 NO2, does not. It is close, but the residual difference is unexplained, so it renders on the page marked as not reproducing rather than being dropped or having a tolerance widened around it. The three that hold are asserted in `site/src/lib/deq-data.ts:491`, and drift outside tolerance fails the build.
