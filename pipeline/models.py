...
class DiseaseData(StrictModel):
    ...
    name: str = Field(min_length=2)
    disease_category: str = Field(min_length=2)
    icd11_code: str = Field(min_length=2)
    icd10_codes: list[str] = Field(min_length=1)
    primary_pathophysiology: str = Field(min_length=12)
    tissue_system: list[str] = Field(min_length=1)
    inheritance_pattern: str = Field(min_length=3)
    prevalence_global: str = Field(min_length=3)
    heritability_estimate: str = Field(min_length=3)
    genetic_architecture_type: str = Field(min_length=3)
    major_genes: list[str] = Field(min_length=1)
    common_risk_variants: list[str] = Field(default_factory=list)
    rare_high_penetrance_variants: list[str] = Field(default_factory=list)
    epigenetic_factors: list[str] = Field(default_factory=list)
    transcriptomic_signatures: list[str] = Field(default_factory=list)
    proteomic_markers: list[str] = Field(default_factory=list)
    metabolomic_markers: list[str] = Field(default_factory=list)
    gene_environment_interactions: list[str] = Field(default_factory=list)
    polygenic_risk_score_supported: bool = False
    validated_prs_studies: list[str] = Field(default_factory=list)
    environmental_exposures: list[str] = Field(min_length=1)
    modifiable_risk_factors: list[str] = Field(default_factory=list)
    nonmodifiable_risk_factors: list[str] = Field(default_factory=list)
    biomarkers_clinical: list[str] = Field(default_factory=list)
    age_of_onset_distribution: str = Field(min_length=6)
    sex_bias: str = Field(min_length=3)
    ancestry_specific_risks: list[str] = Field(default_factory=list)
    comorbidities: list[str] = Field(default_factory=list)
    causal_pathways: list[str] = Field(min_length=1)
    ...
    model_features_for_ml: list[str] = Field(min_length=1)
...
class ExposureData(StrictModel):
    ...
    slug: str = Field(pattern=SLUG_PATTERN)
    name: str = Field(min_length=2)
    exposure_category: str = Field(min_length=2)
    biological_mechanism: str = Field(min_length=8)
    interacting_genes: list[str] = Field(min_length=1)
    effect_direction: Direction
    dose_response_model: str = Field(min_length=3)
    temporal_sensitivity: str = Field(min_length=3)
    ancestry_modifiers: list[str] = Field(default_factory=list)
