  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <article className="space-y-10">
        ...
        <header>
          <h1 className="text-h1 text-surface-white">{exposure.name}</h1>
        </header>

        <section aria-labelledby="definition-heading">
          <h2 id="definition-heading" className="text-h2 text-surface-white mb-3">
            Exposure Definition
          </h2>
          <p className="text-cool-light mb-4">{exposure.definition}</p>
          {exposure.proxies && exposure.proxies.length > 0 && (
            <div>
              <h3 className="text-h3 text-surface-white mb-2">Proxies</h3>
              <table className="min-w-full text-sm border border-white/[0.06] rounded-sm overflow-hidden">
                <thead className="bg-white/[0.03]">
                  <tr>
                    <th className="text-left px-4 py-2 font-medium text-surface-white">Name</th>
                    <th className="text-left px-4 py-2 font-medium text-surface-white">Unit</th>
                    <th className="text-left px-4 py-2 font-medium text-surface-white">Measurement</th>
                    <th className="text-left px-4 py-2 font-medium text-surface-white">Data source</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-white/[0.06]">
                  {exposure.proxies.map((p, i) => (
                    <tr key={i} className="hover:bg-white/[0.04]">
                      <td className="px-4 py-2 text-cool-light">{p.name}</td>
                      <td className="px-4 py-2 text-cool-mid">{p.unit}</td>
          ...
          <section aria-labelledby="data-sources-heading">
            ...
            <div className="card text-sm">
              <p className="text-cool-light">
                <strong className="text-surface-white">Data source:</strong>{" "}
                {exposure.exposure_distribution.data_source}
              </p>
              <p className="text-cool-light">
                <strong className="text-surface-white">Geographic scope:</strong>{" "}
              ...
              {exposure.exposure_distribution.summary_stats &&
                Object.keys(exposure.exposure_distribution.summary_stats).length > 0 && (
                  <div className="mt-2 text-cool-light">
                    <strong className="text-surface-white">Summary stats:</strong>{" "}
        ...
        <section aria-labelledby="systems-affected-heading">
          <h2 id="systems-affected-heading" className="text-h2 text-surface-white mb-3">
            ...
            <div className="space-y-3">
              {exposure.systems_affected.map((s, i) => (
                <div key={i} className="card">
                  <h3 className="text-h3 text-surface-white mb-1">{s.system}</h3>
                  <p className="text-sm text-cool-light">{s.mechanism_summary}</p>
                  {s.evidence_strength != null && (
                    <p className="text-xs text-cool-mid mt-1">
        ...
        <section aria-labelledby="sensitive-windows-heading">
          <h2 id="sensitive-windows-heading" className="text-h2 text-surface-white mb-3">
        ...
        <section aria-labelledby="gxe-heading">
          <h2 id="gxe-heading" className="text-h2 text-surface-white mb-3">
            GxE Highlights
          </h2>
        ...
        <section aria-labelledby="tissues-heading">
          <h2 id="tissues-heading" className="text-h2 text-surface-white mb-3">
          ...
          {exposure.tissues && exposure.tissues.length > 0 ? (
            <div className="flex flex-wrap gap-2">
              {exposure.tissues.map((t, i) => (
                ...
                >
                  <span className="font-medium text-surface-white">{t.name}</span>
                  {t.effect_type && (
                    <span className="text-cool-mid ml-1">— {t.effect_type}</span>
                  )}
                </div>
              ))}
            </div>
