...
export default async function PathwayDetailPage({
  ...
  return (
    <article className="space-y-10">
      ...
      <header>
        <h1 className="text-h1 text-genarch-text">{pathway.name}</h1>
      ...
      <section aria-labelledby="overview-heading">
        ...
        <p className="text-genarch-text">{pathway.summary}</p>
      </section>

      {/* Environmental Triggers */}
      <section aria-labelledby="environmental-triggers-heading">
        <h2 id="environmental-triggers-heading" className="text-h2 text-genarch-text mb-3">
          Environmental Triggers
        </h2>
            ...
            <table className="min-w-full text-sm border border-gray-200 rounded-sm overflow-hidden">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="text-left px-4 py-2 font-medium">Trigger type</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                {pathway.environmental_triggers.map((et, i) => (
                  <tr key={i} className="hover:bg-gray-50/50">
                    ...
                    <td className="px-4 py-2">{et.trigger_type}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <section aria-labelledby="genetic-modulation-heading">
        <h2 id="genetic-modulation-heading" className="text-h2 text-genarch-text mb-3">
          Genetic Modulation Points
        </h2>
        <div className="space-y-4">
          {pathway.key_genes && pathway.key_genes.length > 0 && (
            <div>
              <h3 className="text-h3 text-genarch-text mb-2">Key genes</h3>
                ...
                <ul className="space-y-2">
                  {pathway.regulatory_checkpoints.map((rc, i) => (
                    <li key={i} className="card">
                      <span className="font-medium">{rc.node}</span>
                      {rc.modulator_genes &&
                      ...
                      {rc.potential_therapeutic_target && (
                        <span className="ml-2 text-xs bg-genarch-action/20 text-genarch-text px-2 py-0.5 rounded-sm">
                          Therapeutic target
                        </span>
                      )}
                    </li>
        ...
        {pathway.tissue_specificity && pathway.tissue_specificity.length > 0 ? (
          <div className="flex flex-wrap gap-2">
            {pathway.tissue_specificity.map((ts, i) => (
              ...
              >
                <span className="font-medium text-genarch-text">{ts.tissue}</span>
      ...
      <section aria-labelledby="disease-relevance-heading">
        <h2 id="disease-relevance-heading" className="text-h2 text-genarch-text mb-3">
          Disease Relevance
        </h2>
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            <h3 className="text-h3 text-genarch-text mb-2">Linked diseases</h3>
          ...
          <div>
            <h3 className="text-h3 text-genarch-text mb-2">Linked exposures</h3>
      ...
      <section aria-labelledby="diagram-heading">
        <h2 id="diagram-heading" className="text-h2 text-genarch-text mb-3">
          Pathway Diagram
        </h2>
        <div className="rounded-sm border-2 border-dashed border-gray-200 bg-gray-50/50 p-12 text-center">
      ...
      {/* Evidence Nodes — represented as environmental triggers + key genes + linked entities */}
      <section aria-labelledby="evidence-nodes-heading">
        <h2 id="evidence-nodes-heading" className="text-h2 text-genarch-text mb-3">
          Evidence Nodes
        </h2>
        <p className="text-sm text-genarch-subtext mb-2">
